...
import type { Task, TaskStatus, Priority, ActivityLog, Attachment, ModuleCategory, Subtask } from '../types';
...
export const QuickUpdateModal: React.FC<QuickUpdateModalProps> = ({
  ...
  const handleSaveSubtaskTitle = (subId: string) => {
    ...
    setEditSubtasks(
      editSubtasks.map((st) =>
        st.id === subId ? { ...st, title: editingSubtaskTitle.trim() } : st
      )
    );
    setEditingSubtaskId(null);
  };
  ...
  const handleUpdateSubtaskField = (
    subId: string,
    field: 'assignee' | 'startDate' | 'dueDate' | 'title',
    value: string
  ) => {
    setEditSubtasks(
      editSubtasks.map((st) => (st.id === subId ? { ...st, [field]: value || undefined } : st))
    );
  };
                  ...
                  {editSubtasks.map((st) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {st.dueDate && (
                          <span className="text-[10px] text-slate-600 font-bold bg-white px-1.5 py-0.5 rounded border border-slate-200 shadow-2xs">
                            📅 {st.startDate ? `${st.startDate.slice(5)} ➔ ` : ''}{st.dueDate.slice(5)}
                          </span>
                        )}
                        {st.assignee && (
          ...
          <form onSubmit={handleEditSubmit} className="p-5 sm:p-6 space-y-4 text-sm overflow-y-auto">
            ...
            <div className="space-y-2.5 p-3.5 bg-indigo-50/40 rounded-2xl border border-indigo-100">
              ...
              <div className="space-y-2 pt-1">
                <span className="text-[11px] text-slate-500 font-semibold block">
                  รายการงานย่อย (คลิกติ๊กถูกเพื่อเปลี่ยนสถานะเสร็จ หรือแก้ไขวันเริ่ม/สิ้นสุด):
                </span>
                {editSubtasks.length > 0 ? (
                  editSubtasks.map((st) => (
                    <div
                      key={st.id}
                      className={`p-2.5 px-3 rounded-2xl border text-xs transition-all space-y-2 ${
                        st.completed
                          ? 'bg-emerald-50/40 border-emerald-200/80 text-slate-400'
                          : 'bg-white hover:bg-slate-50/70 border-slate-200/90 text-slate-800 shadow-2xs'
                      }`}
                    >
                      {/* Top line: Checkbox, Title (Editable), Assignee & Delete */}
                      <div className="flex items-center justify-between gap-2">
                          ...
                          {editingSubtaskId === st.id ? (
                            <div className="flex items-center gap-1.5 flex-1">
                              <input
                                ...
                                className="flex-1 px-2.5 py-1 bg-white border border-indigo-400 rounded-lg text-xs font-semibold text-slate-900 outline-none ring-2 ring-indigo-100"
                                autoFocus
                              />
                              <button
                                type="button"
                                onClick={() => handleSaveSubtaskTitle(st.id)}
                                className="px-2 py-1 bg-indigo-600 text-white rounded-lg text-[10px] font-bold shrink-0 cursor-pointer"
                              >
                                บันทึก
                              </button>
                            </div>
                            ...
                            <div className="flex items-center gap-1.5 min-w-0 flex-1">
                              <span
                                ...
                                className={`truncate font-semibold cursor-pointer hover:text-indigo-600 transition-colors ${
                                  st.completed ? 'line-through text-slate-400' : 'text-slate-800'
                                }`}
                                title="คลิกเพื่อแก้ไขชื่องานย่อย"
                              >
                                {st.title}
                              </span>
                              <button
                                type="button"
                                onClick={() => handleStartEditSubtask(st)}
                                className="text-slate-300 hover:text-indigo-600 p-0.5 rounded transition-colors cursor-pointer shrink-0"
                        ...
                        {/* Assignee select & Delete button */}
                        <div className="flex items-center gap-2 shrink-0">
                          <select
                            value={st.assignee || 'ยังไม่ระบุ'}
                            onChange={(e) => handleUpdateSubtaskField(st.id, 'assignee', e.target.value)}
                            className="text-[11px] font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 px-2 py-0.5 rounded-full outline-none cursor-pointer transition-colors"
                            title="เลือกเปลี่ยนผู้รับผิดชอบงานย่อยนี้"
                          >
                      ...
                      {/* Bottom line: Start Date & Due Date (Deadline) */}
                      <div className="flex flex-wrap items-center justify-between gap-2 pl-6 pt-1 border-t border-slate-100/80 text-[11px]">
                        <div className="flex items-center gap-2 flex-wrap text-slate-500">
                          {/* วันที่เริ่ม */}
                          <div className="flex items-center gap-1">
                            <Calendar className="w-3 h-3 text-indigo-500 shrink-0" />
                            <span className="text-slate-500 font-semibold text-[10px]">วันเริ่ม:</span>
                            <input
                              type="date"
                              value={st.startDate || ''}
                              onChange={(e) => handleUpdateSubtaskField(st.id, 'startDate', e.target.value)}
                              className="px-2 py-0.5 bg-slate-50 hover:bg-white focus:bg-white border border-slate-200 focus:border-indigo-500 rounded-md text-[11px] font-medium text-slate-700 outline-none cursor-pointer transition-all"
                              title="กำหนดวันเริ่มงานย่อย"
                            />
                          </div>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-rose-600 font-semibold text-[10px]">เดดไลน์:</span>
                            <input
                              type="date"
                              value={st.dueDate || ''}
                              onChange={(e) => handleUpdateSubtaskField(st.id, 'dueDate', e.target.value)}
                              className="px-2 py-0.5 bg-slate-50 hover:bg-white focus:bg-white border border-slate-200 focus:border-rose-400 rounded-md text-[11px] font-medium text-slate-700 outline-none cursor-pointer transition-all"
                              title="กำหนดวันสิ้นสุด / เดดไลน์งานย่อย"
                            />
                          </div>
                          ...
                          {st.dueDate && (
                            ...
                            >
                              ครบกำหนด: {formatThaiDate(st.dueDate).slice(0, -5)}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              {/* Add subtask input row with dates & assignee */}
              <div className="p-3 bg-white rounded-2xl border border-slate-200 space-y-2 pt-2.5">
                <div className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                  <Plus className="w-3 h-3 text-indigo-600" /> เพิ่มงานย่อยใหม่
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="+ พิมพ์ชื่องานย่อย (เช่น คิดราคา / GP, จัดทำ Sale kit)..."
                    className="flex-1 px-3 py-2 bg-slate-50 focus:bg-white border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-400"
                  />

                  <select
                    value={newSubtaskAssignee}
                    onChange={(e) => setNewSubtaskAssignee(e.target.value)}
                    className="px-2.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shrink-0"
                    title="เลือกผู้รับผิดชอบงานย่อยนี้"
                  >
                    {Array.from(new Set(['Thampapon', 'Arthit', 'Muk', ...availableUsers])).map((u) => (
                      <option key={u} value={u}>
                        ผู้ดูแล: {u}
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-slate-100">
                  <div className="flex items-center gap-3 flex-wrap text-xs">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-slate-500 font-bold">วันเริ่มงาน:</span>
                      <input
                        type="date"
                        value={newSubtaskStartDate}
                        onChange={(e) => setNewSubtaskStartDate(e.target.value)}
                        className="px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>

                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-rose-600 font-bold">วันสิ้นสุด (เดดไลน์):</span>
                      <input
                        type="date"
                        value={newSubtaskDueDate}
                        onChange={(e) => setNewSubtaskDueDate(e.target.value)}
                        className="px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 focus:outline-none focus:ring-1 focus:ring-rose-500"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shrink-0 shadow-xs cursor-pointer flex items-center gap-1"
                  >
